...
function createProposal(uint256 _nftTokenId) external nftHolderOnly returns (uint256) {
    ...
    // return the index of the newaly created proposal which is numProposals - 1
    return numProposals - 1;
}

function voteOnProposal(uint256 _proposalId, Vote vote) external activeProposalOnly(_proposalId) {
...
function executeProposal(uint256 proposalId) external nftHolderOnly inactiveProposalOnly(proposalId) {
    ...
    if(proposal.yayVotes > proposal.nayVotes) {
        ...
        require(nftMarketplace.available(proposal.nftTokenId), "NFT is unavailable");
